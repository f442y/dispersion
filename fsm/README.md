...
* 🔭 [**Control Subsystem (`control/`)**](../control-plane/README.md) — Register atomic executors into `DefaultControlPlane` via `.asInspectableMachine()`.
* 📡 [**Event Subsystem (`event/`)**](../event/README.md) — Telemetry events emitted during state transitions.
* 🧪 [**Testing Framework (`testing/`)**](../testkit/README.md) — Test atomic machines with `DispersionTestKit`.
